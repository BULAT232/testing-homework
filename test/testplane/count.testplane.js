describe("Товар", () => {
  ...
  it("При повторном добавлении товара в корзину, не увеличивается индикатор количества товаров рядом со ссылкой на корзину", async ({
    browser,
  }) => {
    await browser.url("/hw/store/catalog/0");
    const addToCartBtn = await browser.$(".ProductDetails-AddToCart");
  ...
  it('Если товар уже добавлен в корзину, повторное нажатие кнопки "добавить в корзину" увеличивает его количество и стоимость', async ({
    browser,
  }) => {
    await browser.url("/hw/store/catalog/0");
    const addToCartBtn = await browser.$(".ProductDetails-AddToCart");
